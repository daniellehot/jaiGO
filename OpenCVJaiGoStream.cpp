...
void showImageCV(PvBuffer *imgBuffer){
    //Mat img(imgBuffer->GetImage()->GetWidth(), imgBuffer->GetImage()->GetHeight(), CV_8UC3, imgBuffer->GetDataPointer());
    //imshow("JaiGo Stream", img);
                ...
                case PvPayloadTypeImage:
                    {
                        ...
                        //showImageCV(lBuffer);
                        Mat openCvImage = Mat(lBuffer->GetImage()->GetHeight(), lBuffer->GetImage()->GetWidth(), CV_8U, lBuffer->GetDataPointer());
                        Mat Colorimg;
                        cvtColor(openCvImage, Colorimg, COLOR_BayerRG2RGB);
                        Mat debayered;
                        demosaicing(openCvImage, debayered, COLOR_BayerRG2RGB);
                        
                        imshow("Img", debayered);
                        waitKey(1);
                        break;
                    }
